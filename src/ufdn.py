    ...
    if name == 'vae':
        ...
        for layer,para in enumerate(parameter['encoder']):
            if para[0] == 'conv':
                ...
                if layer==0:
                    init_dim = input_dim
                next_dim,bn,act = para[1:4]
                act = get_act(act)
                enc_list.append((para[0],(init_dim, next_dim,bn,act)))
                init_dim = next_dim
            else:
        ...
        for layer,para in enumerate(parameter['decoder']):
            if para[0] == 'conv':
                ...
                init_dim = next_dim
            if para[0] == 'fc':
                next_dim,bn,act,insert_code = para[1:5]
                act = get_act(act)
                dec_list.append((para[0],(init_dim, next_dim,bn,act),insert_code)) 
                init_dim = next_dim
            else:
        ...
        dnet_list = []
        init_dim = input_dim
        cur_img_size = img_size # This is the only place img_size is used
        reshaped = False
